...
def tweetToneDescriberGPT(apikey, model, inputAuthorText):
    messages =[]
...
def authorTweetFinderGPT(apikey, model, inputAuthorText):
...
def createTweet(model, apikey, inputAuthorText, inputText, selectOutputFormat, customInstruction):
    messages =[]
    openai.api_key = apikey
    MODEL = model

    token_count1, tone = tweetToneDescriberGPT(apikey, model, inputAuthorText)
    token_count2, authors = authorTweetFinderGPT(apikey, model, inputAuthorText)
    ...
    TASK:
    - You are an AI Bot that is very good at transforming the input text, denoted as INPUT TEXT, into a series of Tweets in the style of the authors.
    - The list of Authors is provided under LIST OF AUTHORS, the tone of the authors is denoted as AUTHOR TONE and the tweets of the author to mimic, denoted as AUTHOR TONE.
    - Use the AUTHOR TONE to generate tweets from the INPUT TEXT but do not use the AUTHOR TWEETS in the output.

    IMPORTANT INSTRUCTIONS:
    - Your output should consist of a series of tweets based on the INPUT TEXT only, and written in the style of the AUTHOR TWEETS.
    - Do not change the meaning of the input text, only transform it into tweets.
    - Under any circumstances, Do not use the AUTHOR TWEETS in your output.
